...
def train_vqvae(epochs, K_val, D_val, data_root='./data'):
...
def multi_agent_game(vqvae, aim_dict, rounds=5, aim_seq_len=2, K_val=16, 
                     ...
                     rl_train_on_mnist_train=False, return_agents=False, data_root='./data'):
    ...
    transform = transforms.ToTensor()
    game_data = torchvision.datasets.MNIST(
        root=data_root, train=rl_train_on_mnist_train, download=True, transform=transform
    )
    ...
    for idx, (_, label) in enumerate(game_data):
    ...
    for i in range(rounds):
        ...
        if not matching_indices:
            idx = random.randint(0, len(game_data)-1)
            x, _ = game_data[idx]
        else:
    ...
    if return_agents:
        return A_rewards_history, B_rewards_history, agentA, agentB
