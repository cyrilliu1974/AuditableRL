...
> **Current status (2026-09-22): evidence-aligned synthesis.** The first sections below preserve the original two-seed, seed-specific-symbolizer pilot as historical evidence. For the current eight-seed shared-symbolizer results, held-out behavioral test, Acrobot arbitration diagnosis, and FQE-GPI quality checks, use the dated update near the end of this report and the root `README.md`. The current paper claim is bounded existence: the tested workflow provides replay checks, provenance-visible rule composition, and a separate causal-symbol result in a designed task. Generality and complete auditability of arbitrary RL training are not established.

### Current result in brief

- Shared frozen symbolizers yield mean pairwise rule Jaccard of 0.5572 (CartPole) and 0.0956 (Acrobot). Yet CartPole seed 11/29 action agreement on fresh held-out raw states is only 51.36% (95% interval 50.48%–52.26%) despite Jaccard 0.5833; symbolic overlap is not behavioral consensus.
- Acrobot rule fusion scores -443.37 versus -500 for the best single actor. Its decisions are 95.70% covered, 89.24% conflicting, and 4.30% blind spots. The arbitration ablations are unstable, and random source-rule selection unexpectedly outperforms the tested deterministic variants; this mechanism remains open.
- Hash, provenance, and environment replay checks support trace-level auditability for the recorded runs and tested software setup. AIM interventions support causal influence of a learned symbol in the specific private-target task, not a general natural-language explanation of neural policies.
- FQE-GPI fails key quality checks and is kept separate from the primary fusion result.

The study is therefore ready to be presented as a bounded existence result with explicit counterevidence. Remaining work is future research rather than a prerequisite for claiming universal auditability; submission still requires ordinary venue and coauthor review.
...
- “A symbolic rule intersection, including the earlier two-seed pilot, establishes a universal, semantically identical policy core.”
...
## Future work (outside the present paper's claim)
...
The current results should be submitted with the stated limits; these four items are follow-up research, not missing evidence for the bounded existence claim. Venue formatting and author review remain editorial steps.

## Reproducibility artifacts

The following are canonical paths in the current repository layout; older log entries and the historical pilot section may retain paths from the earlier nested layout.

- Shared-symbolizer overlaps: `results/shared_multienv_gpi/CartPole-v1/shared_symbolizer_rule_overlap.json`, `Acrobot-v1/shared_symbolizer_rule_overlap.json`.
- Experiment B: `results/shared_multienv_gpi/CartPole-v1/behavioral_agreement_experiment_B.json`.
- Acrobot fusion, arbitration, and random-action controls: `results/shared_multienv_gpi/Acrobot-v1/grammar_fusion_diagnostics.json`, `random_action_control.json`.
- Artifact audits: `results/shared_multienv_gpi/CartPole-v1/artifact_audit.json`, `Acrobot-v1/artifact_audit.json`.
- FQE-GPI diagnostics: `results/shared_multienv_gpi/CartPole-v1/gpi_fqe/`, `Acrobot-v1/gpi_fqe/`.
- Detailed experiment chronology: `paper/AI_Grammar_Induction_Experiment_Log.md` and `paper/AIM_Auditability_Research_Log.md`.
- Compact-package hashes and sizes: `results/RESULTS_MANIFEST.json`.
