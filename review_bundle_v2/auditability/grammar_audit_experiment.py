            ...
            if count >= self.min_support and confidence >= self.min_confidence:
                ...
                rules[condition] = {
                    ...
                    "source_step_ids_sha256": hashlib.sha256(
                        canonical_json(source_step_ids).encode("utf-8")
                    ).hexdigest(),
                }
        return rules
...
def train_one(
    ...
    constraint_rule_snapshots: dict[str, dict] = {}
    ...
    with trace_path.open("w", encoding="utf-8", newline="\n") as trace, update_path.open(
        ...
        for episode in range(episodes):
            ...
            while not (terminated or truncated):
                ...
                constraint_rule = None if applied_rule is None else {
                    "action": int(applied_rule["action"]),
                    "support": int(applied_rule["support"]),
                    "condition_count": int(applied_rule["condition_count"]),
                    "confidence": float(applied_rule["confidence"]),
                    "mean_reward": float(applied_rule["mean_reward"]),
                    "source_step_count": len(applied_rule["source_step_ids"]),
                    "source_step_ids_sha256": applied_rule["source_step_ids_sha256"],
                }
                if applied_rule is not None:
                    snapshot = {
                        "state_symbol": "STATE_" + "_".join(map(str, state_symbol)),
                        "action": int(applied_rule["action"]),
                        "support": int(applied_rule["support"]),
                        "condition_count": int(applied_rule["condition_count"]),
                        "confidence": float(applied_rule["confidence"]),
                        "mean_reward": float(applied_rule["mean_reward"]),
                        "source_step_ids": list(applied_rule["source_step_ids"]),
                    }
                    snapshot_id = hashlib.sha256(canonical_json(snapshot).encode("utf-8")).hexdigest()
                    constraint_rule_snapshots[snapshot_id] = snapshot
                    constraint_rule["snapshot_id"] = snapshot_id
    ...
    if arm == "grammar_constrained":
        ...
        snapshot_path.write_text(json.dumps(constraint_rule_snapshots, sort_keys=True, separators=(",", ":")) + "\n", encoding="utf-8")
