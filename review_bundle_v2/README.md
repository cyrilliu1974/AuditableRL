...
```text
C:\AI\RL\
|-- README.md                         # This experiment and publication guide
|-- .gitignore                        # Excludes local environments, reference clones, and full runs
|-- auditability\                     # Experiment runners, validators, and diagnostic analyses
|-- requirements.txt                  # Python dependencies for reproduction
|-- ai_grammar_induction.py            # Legacy wrapper; routes to the validated experiment path
|-- vqvae_agents_AIM.py                # Local AIM source snapshot used by the controlled-symbol study
|-- aim_dictionary_json.py             # Local AIM dictionary source snapshot
|-- behavioral_agreement.py            # User-provided reference implementation for Experiment B
|-- manuscript\                       # English paper draft and revisions
|-- references\                       # Local research-code clones; not vendored into Git
|-- results\                           # Compact review package, suitable for GitHub inspection
|-- runs\                              # Full local traces, checkpoints, and run outputs (multi-GB)
`-- paper\                             # Plain-text research logs and reports only
```
...
The full `runs/` tree is retained locally for trace replay and provenance inspection. It is intentionally excluded from regular Git commits because it is multi-gigabyte, including JSONL traces and model checkpoints. A curated 16-file, approximately 0.51 MB review package under `results/` contains the multi-seed summaries, overlap tables, audit reports, FQE diagnostics, arbitration ablations, the uniform random-action control, Experiment B, and the action-level MC comparison. `results/RESULTS_MANIFEST.json` records each package file’s source path, byte count, and SHA-256. The local full run tree remains the authoritative source for that package.
...
Detailed hypotheses, negative results, implementation corrections, citations, and claim limits are in `paper/AI_Grammar_Induction_Experiment_Log.md`. The broader study summary is `paper/20260922_RL_Grammar_Fusion_Research_Report.md`. The manuscript in `manuscript/rl_rule_fusion_draft.md` is a draft for author review, not a submission.
